  ...
  return (
    <div style={{ width: '100%', height: '100%' }}>
      <div className="map-controls-overlay">
        <button onClick={() => setShowHeatmap(!showHeatmap)} className={`map-control-btn ${showHeatmap ? 'active' : ''}`}>
          {showHeatmap ? 'Hide Heatmap' : 'Show Heatmap'}
        </button>
        <button onClick={() => setShowRisk(!showRisk)} className={`map-control-btn ${showRisk ? 'active' : ''}`}>
          {showRisk ? 'Hide Risk Zones' : 'Show Risk Zones'}
        </button>
        <button onClick={() => setShowBlackspots(!showBlackspots)} className={`map-control-btn ${showBlackspots ? 'active' : ''}`} style={{ borderColor: showBlackspots ? '#ef4444' : '' }}>
