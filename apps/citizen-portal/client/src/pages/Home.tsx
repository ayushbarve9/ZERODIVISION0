...
export default function Home() {
  const activeIssue = mockIssues.length > 0 ? mockIssues[0] : null;
  return (
    <CivicShell
    ...
    >
      <section className="dashboard-intro">
        <div className="intro-copy"><p>Keep your neighborhood moving forward. <strong>{mockIssues.length} active reports</strong> in your district.</p><Link href="/complaints" className="text-link">View your complaint history <ArrowUpRight size={15} /></Link></div>
      ...
      <section className="metric-row" aria-label="Your activity summary">
        <div className="metric-card metric-card-featured"><div className="metric-top"><span className="metric-label">Reports this year</span><TrendingUp size={16} /></div><strong>{mockIssues.length}</strong><span className="metric-foot positive"><ArrowUpRight size={13} /> Live telemetry</span></div>
        <div className="metric-card"><div className="metric-top"><span className="metric-label">Resolved</span><Check size={16} /></div><strong>0</strong><span className="metric-foot">100% SLA target</span></div>
        <div className="metric-card"><div className="metric-top"><span className="metric-label">Neighbors reached</span><ThumbsUp size={16} /></div><strong>0</strong><span className="metric-foot">Community network</span></div>
      </section>
          ...
          <section className="section-block">
            ...
            {activeIssue ? (
              <Link href={`/issues/${activeIssue.id}`} className="active-issue-card">
                <div className="active-issue-image"><img src={activeIssue.image} alt="Report preview" /><span className="image-stamp">{activeIssue.id}</span></div>
              ...
              <div className="active-issue-card" style={{ padding: '28px 20px', textAlign: 'center', background: 'rgba(35, 134, 137, 0.1)', border: '1px dashed rgba(35, 134, 137, 0.5)', borderRadius: '16px' }}>
                <h3 style={{ color: '#F2FFF6', marginBottom: '6px', fontSize: '15px' }}>Zero Active Reports</h3>
                <p style={{ color: 'rgba(242, 255, 246, 0.7)', fontSize: '13px', marginBottom: '16px' }}>You have not submitted any reports yet. Notice an infrastructure hazard on your block?</p>
                <Link href="/report" className="button button-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', margin: '0 auto', textDecoration: 'none' }}>
          ...
          <section className="section-block activity-block">
            ...
            <div className="activity-list">
              {mockIssues.length > 1 ? (
                ...
                <div style={{ padding: '24px', textAlign: 'center', color: 'rgba(242, 255, 246, 0.5)', fontSize: '12px' }}>
                  No recent community reports found in your district.
                </div>
              )}
            </div>
          </section>
